...
use App\Services\Zatca\Mappers\Validators\InvoiceValidator;
// use App\Services\Zatca\Mappers\Validators\InvoiceAmountValidator;
...
class InvoiceMapper
{
    ...
    private function mapUBLExtensions(array $data): UBLExtensions
    {
        ...
        $signatureInfo = (new \AApp\Services\Zatca\SignatureInformation())
        ...
        foreach ($data['allowanceCharges'] as $allowanceCharge) {
            ...
            if (isset($allowanceCharge['taxCategories']) && is_array($allowanceCharge['taxCategories'])) {
                foreach ($allowanceCharge['taxCategories'] as $taxCatData) {
                    $taxCategories[] = (new \AApp\Services\Zatca\TaxCategory())
                        ->setPercent($taxCatData['percent'] ?? 15)
                        ->setTaxScheme(
                            (new \AApp\Services\Zatca\TaxScheme())
            ...
            $allowanceCharges[] = (new \AApp\Services\Zatca\AllowanceCharge())
     ...
     * @return \AApp\Services\Zatca\Delivery The mapped Delivery object.
     */
    private function mapDelivery(array $data): \AApp\Services\Zatca\Delivery
    {
        return (new \AApp\Services\Zatca\Delivery())
    ...
    private function mapTaxTotal(array $data): TaxTotal
    {
        ...
        if (isset($data['subTotals']) && is_array($data['subTotals'])) {
            foreach ($data['subTotals'] as $subTotal) {
                ...
                $taxScheme = (new \AApp\Services\Zatca\TaxScheme())
                    ->setId($taxSchemeData['id'] ?? "VAT");
                
                // Build the TaxCategory object using the extracted data.
                $taxCategory = (new \AApp\Services\Zatca\TaxCategory())
                    ->setPercent($percent)
                ...
                $taxSubTotal = (new \AApp\Services\Zatca\TaxSubTotal())
